...
export default function SouvenirListing({ souvenirs, onSouvenirClick }) {
  ...
  return (
    <div className="container mx-auto px-6 md:px-12 py-12 mt-20 bg-gradient-to-br from-white via-slate-50 to-slate-100 rounded-xl shadow-md">
      {/* Header */}
      <div className="text-center mb-10">
        <h1 className="text-4xl font-bold text-slate-800 mb-3">🎁 Regional Souvenirs</h1>
        <p className="text-lg text-slate-600 max-w-2xl mx-auto">
          Discover authentic souvenirs from around the world — rich in culture, crafted with love.
        </p>
      </div>
      ...
      <div className="mb-8">
        <div className="flex flex-col lg:flex-row gap-4 items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <button
              onClick={() => setShowFilters(!showFilters)}
              className="flex items-center gap-2 px-4 py-2 bg-indigo-50 text-indigo-700 border border-indigo-300 rounded-lg hover:bg-indigo-100 transition-all duration-200"
            >
              <Filter className="w-4 h-4" />
              Filters
            </button>
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        {/* Filters Sidebar */}
        {showFilters && (
          <div className="lg:w-80 w-full flex-shrink-0 transition-all duration-300">
            <FilterSidebar
              regions={regions}
              categories={categories}
              selectedRegion={selectedRegion}
        ...
        <div className="flex-1">
          {filteredSouvenirs.length === 0 ? (
            <div className="text-center py-16 bg-white rounded-lg shadow-inner border border-dashed border-gray-300">
              <div className="text-rose-400 mb-4">
                <Search className="w-16 h-16 mx-auto" />
              </div>
              <h3 className="text-xl font-semibold text-slate-800 mb-2">No souvenirs found</h3>
